...
FILE_SUFFIX="v${RONDB_VERSION_NO_DOT}_m${NUM_MGM_NODES}_g${NODE_GROUPS}_r${REPLICATION_FACTOR}_my${NUM_MYSQL_NODES}_api${NUM_API_NODES}${USER_SUFFIX}"
...
if [ "$NUM_API_NODES" -gt 0 ]; then
    for CONTAINER_NUM in $(seq "$NUM_API_NODES"); do
        SERVICE_NAME="api_${CONTAINER_NUM}${USER_SUFFIX}"
        template="$(service-template)"
